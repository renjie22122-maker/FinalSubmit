...
# Match both sat_* and satellite_* naming conventions
...
def parse_existing_bounds(sat_dir: Path) -> tuple[float, float, float, float]:
    ...
    if not lats or not lons:
        raise RuntimeError(f"No sat_*.png or satellite_*.png tiles found in {sat_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Resample satellite tiles with 10% overlap")
    parser.add_argument("--source-dir", type=Path, default=Path("LondonDataSet/London/satellite_overlap50"))
    parser.add_argument("--output-dir", type=Path, default=Path("LondonDataSet/London/satellite_overlap10"))
    ...
    # 10% overlap => stride = 90% of tile size
    stride_px = int(args.tile_size * 0.9)
    ...
    print(f"tile_size={args.tile_size} stride_px={stride_px} overlap=10%")
