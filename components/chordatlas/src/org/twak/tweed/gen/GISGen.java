...
public class GISGen  extends LineGen3d implements ICanSave {
	...
	public enum SelectionMeshSource {
		LOADED_WORKSPACE( "loaded workspace mesh (legacy)" ),
		SATELLITE_TILES( "satellite tiles on demand" ),
		BIG_IMAGE_ON_DEMAND( "big image app192 on demand" );

		final String label;
		SelectionMeshSource( String label ) { this.label = label; }
	...
	transient Map<String, List<BlockGen>> generatedSelectionMeshes = new HashMap<>();
...
//	@Deprecated transient Matrix4d toOrigin;
	...
	/** Persisted per GIS layer; absent in older tweed.xml files therefore defaults safely to false. */
	@Deprecated
	public boolean satelliteMeshOnSelect = false;
	/** Three-way source selector. Null in old XML is migrated from satelliteMeshOnSelect. */
	public SelectionMeshSource selectionMeshSource;
	private transient SelectedBlockMeshService selectedBlockMeshService;
	private transient JLabel selectedBlockMeshStatusLabel;
	private transient String selectedBlockMeshStatus = "Disabled: Select uses mesh already loaded in the workspace.";
	...
	public void onLoad( Tweed tweed ) {
		...
		generatedSelectionMeshes = new HashMap<>();
		selectedBlockMeshService = null;
		selectedBlockMeshStatusLabel = null;
		SelectionMeshSource source = currentSelectionMeshSource();
		selectedBlockMeshStatus = source == SelectionMeshSource.LOADED_WORKSPACE
				? "Ready: Select uses mesh already loaded in the workspace."
				: "Ready: right-click a GIS footprint to generate " + source + ".";
...
//	public static Mode mode = Mode.RENDER_SELECTED_BLOCK;
	...
	protected void polyClicked( int callbackI ) {
		...
		if (tweed.frame.selectedGen instanceof GenHandlesSelect) {
			...
			switch ( currentSelectionMeshSource() ) {
			case BIG_IMAGE_ON_DEMAND:
				importGeneratedMesh( callbackI, SelectedBlockMeshService.ModelSource.BIG_IMAGE );
				break;
			case SATELLITE_TILES:
				importGeneratedMesh( callbackI, SelectedBlockMeshService.ModelSource.SATELLITE_TILES );
				break;
...
//			}, true);
	...
	public JComponent getUI() {
		...
		JComboBox<SelectionMeshSource> source = new JComboBox<>( SelectionMeshSource.values() );
		source.setSelectedItem( currentSelectionMeshSource() );
		source.addActionListener( event -> {
			selectionMeshSource = (SelectionMeshSource) source.getSelectedItem();
			satelliteMeshOnSelect = selectionMeshSource != SelectionMeshSource.LOADED_WORKSPACE;
			setSelectedBlockMeshStatus( selectionMeshSource == SelectionMeshSource.LOADED_WORKSPACE
					? "Ready: Select uses mesh already loaded in the workspace."
					: "Ready: right-click a GIS footprint to generate " + selectionMeshSource + "." );
		} );
		out.add( new JLabel( "mesh source on Select" ) );
		out.add( source );
		out.add( new JLabel( "<html>Loaded workspace keeps the original import path. Satellite tiles keeps the existing<br>per-tile bridge. Big image runs app192/640/75% feathering as a separate cached layer.</html>" ) );
		selectedBlockMeshStatusLabel = new JLabel( selectedBlockMeshStatus );
		out.add( selectedBlockMeshStatusLabel );
		
		return out;
	}

	private SelectionMeshSource currentSelectionMeshSource() {
		if ( selectionMeshSource == null )
			selectionMeshSource = satelliteMeshOnSelect
					? SelectionMeshSource.SATELLITE_TILES
					: SelectionMeshSource.LOADED_WORKSPACE;
		satelliteMeshOnSelect = selectionMeshSource != SelectionMeshSource.LOADED_WORKSPACE;
		return selectionMeshSource;
	}

	/**
	 * Used by the picker to let an explicitly selected GIS layer receive an
	 * on-demand footprint click through already loaded generated meshes.
	 */
	public boolean isOnDemandSelectionActive() {
		return currentSelectionMeshSource() != SelectionMeshSource.LOADED_WORKSPACE;
	}

	private void importGeneratedMesh( int index, SelectedBlockMeshService.ModelSource modelSource ) {
		LoopL<Point3d> selected = blocks.get( index );
		if ( selected == null || selected.isEmpty() ) {
			setSelectedBlockMeshStatus( "Failed: the selected GIS block has no footprints." );
			return;
		}

		String baseSelectionId = SelectedBlockMeshService.stableSelectionId( selected );
		String selectionId = modelSource == SelectedBlockMeshService.ModelSource.BIG_IMAGE
				? "big-image-" + baseSelectionId : baseSelectionId;
		List<BlockGen> generated = generatedSelectionMeshes.get( selectionId );
		if ( generated != null ) {
			List<BlockGen> active = generated.stream()
					.filter( block -> tweed.frame.getGensOf( BlockGen.class ).contains( block ) )
					.collect( Collectors.toList() );
			if ( !active.isEmpty() ) {
				setSelectedBlockMeshStatus( "Already loaded " + active.size()
						+ " independent building(s): " + selectionId );
				tweed.frame.setSelected( active.get( 0 ) );
				return;
			}
			generatedSelectionMeshes.remove( selectionId );
		}

		if ( selectedBlockMeshService == null )
			selectedBlockMeshService = new SelectedBlockMeshService( tweed );

		SelectedBlockMeshService.Callback callback = new SelectedBlockMeshService.Callback() {
			@Override
			public void statusChanged( String status ) {
				setSelectedBlockMeshStatus( status );
			}

			@Override
			public void completed( SelectedBlockMeshService.Result result ) {
				if ( !SwingUtilities.isEventDispatchThread() )
					throw new IllegalStateException( "Selection mesh completion must run on the EDT" );
				List<MiniGen> minis = new ArrayList<>();
				List<BlockGen> buildingBlocks = new ArrayList<>();
				List<VertexColorObjGen> vertexColors = new ArrayList<>();
				List<MiniGen> addedMinis = new ArrayList<>();
				List<BlockGen> addedBlocks = new ArrayList<>();
				List<VertexColorObjGen> addedVertexColors = new ArrayList<>();
				try {
					SelectedBlockMeshService.validateBlockDirectory( result.outputDirectory, result.selectionId,
							result.pipelineContractVersion, result.osmPrealign );
					if ( result.buildings.isEmpty() )
						throw new IllegalStateException( "The publication has no independently loadable building" );

					// Validate and construct every layer before exposing any of them. Each MiniGen
					// remains hidden by default; the visible BlockGen is the semantic building
					// used by find profiles, panoramas, features, and later optimisation steps.
					for ( SelectedBlockMeshService.BuildingResult building : result.buildings ) {
						SelectedBlockMeshService.validateMiniMesh( building.miniMeshDirectory );
						MiniGen mini = new MiniGen(
								tweed.makeWorkspaceRelative( building.miniMeshDirectory ), tweed );
						mini.name = building.displayName() + " mesh (hidden)";
						BlockGen block = new BlockGen( building.directory, tweed, building.footprints );
						block.name = building.displayName();
						VertexColorObjGen vertexColor = building.vertexColorObj == null ? null
								: new VertexColorObjGen( building.displayName() + " vertex colour",
										building.croppedObj, building.vertexColorObj, tweed );
						minis.add( mini );
					...
					int coloredCount = 0;
					for ( int buildingIndex = 0; buildingIndex < buildingBlocks.size(); buildingIndex++ ) {
						MiniGen mini = minis.get( buildingIndex );
						BlockGen block = buildingBlocks.get( buildingIndex );
						VertexColorObjGen vertexColor = vertexColors.get( buildingIndex );
						tweed.frame.addGen( mini, false );
						addedMinis.add( mini );
						// Keep the semantic BlockGen for profiles/panoramas/optimisation. When
						// cropped.obj has valid per-vertex RGB, the colour layer is the default
						// display; otherwise preserve the old visible BlockGen behaviour.
						tweed.frame.addGen( block, vertexColor == null );
						addedBlocks.add( block );
						if ( vertexColor != null ) {
							tweed.frame.addGen( vertexColor, true );
							addedVertexColors.add( vertexColor );
							coloredCount++;
						}
					}

					BlockGen primary = buildingBlocks.get( 0 );
					lastMesh.put( index, primary );
					generatedSelectionMeshes.put( result.selectionId,
							Collections.unmodifiableList( new ArrayList<>( buildingBlocks ) ) );
					VertexColorObjGen primaryColor = vertexColors.get( 0 );
					tweed.frame.setSelected( primaryColor == null ? primary : primaryColor );
					setSelectedBlockMeshStatus( "Ready: loaded " + buildingBlocks.size()
							+ " independent building(s), " + coloredCount
							+ " with OBJ vertex colour, from " + result.modelSource.label
							+ " at " + result.outputDirectory );
				} catch ( Throwable error ) {
					for ( int remove = addedVertexColors.size() - 1; remove >= 0; remove-- )
						tweed.frame.removeGen( addedVertexColors.get( remove ) );
					for ( int remove = addedBlocks.size() - 1; remove >= 0; remove-- )
						tweed.frame.removeGen( addedBlocks.get( remove ) );
					for ( int remove = addedMinis.size() - 1; remove >= 0; remove-- )
						tweed.frame.removeGen( addedMinis.get( remove ) );
					failed( new SelectedBlockMeshService.Failure(
							"Generated files failed the final GUI validation: " + error.getMessage(),
							null, result.logFile, result.resultFile ) );
				}
			}

			@Override
			public void failed( SelectedBlockMeshService.Failure failure ) {
				if ( !SwingUtilities.isEventDispatchThread() )
					throw new IllegalStateException( "Selection mesh failure must run on the EDT" );
				String details = failure.details();
				setSelectedBlockMeshStatus( "Failed; see the reported result/log paths." );
				System.err.println( details );
				JOptionPane.showMessageDialog( tweed.frame(), details,
						"Satellite mesh generation failed", JOptionPane.ERROR_MESSAGE );
			}
		};
		if ( modelSource == SelectedBlockMeshService.ModelSource.BIG_IMAGE )
			selectedBlockMeshService.submitBigImage( selected, callback );
		else
			selectedBlockMeshService.submit( selected, callback );
	}

	private void setSelectedBlockMeshStatus( String status ) {
		selectedBlockMeshStatus = status;
		if ( selectedBlockMeshStatusLabel != null )
			selectedBlockMeshStatusLabel.setText( status );
	}
