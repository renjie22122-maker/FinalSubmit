...
class Discriminator(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(self, in_channels=3, cond_channels=0, hidden_channels=512, img_resolution=256):
        super().__init__()
        depth = int(log2(img_resolution) -2)
        d = max(depth - 3, 3)
        layers = [
        ...
        # self.fc = nn.Linear(hidden_channels * int((img_resolution/(2**depth))**2),
        #                               hidden_channels,)
