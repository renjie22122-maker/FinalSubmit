...
public class JointUI extends JPanel {
	...
	SelectedApps sa;
	BlockApp root;
	BlockApp roofReferenceRoot;
	
	public JointUI ( SelectedApps sa, Runnable globalUpdate) {

		this.sa = sa;
		
		int blockRootCount = 0;
		for ( App candidate : sa.findRoots() )
			if ( candidate instanceof BlockApp ) {
				if ( root == null )
					root = (BlockApp) candidate; // preserve the original multi-root editor behaviour
				roofReferenceRoot = (BlockApp) candidate;
				blockRootCount++;
			}
		if ( root == null )
			throw new IllegalArgumentException( "Joint editing requires a selected BlockApp root." );
		if ( blockRootCount != 1 )
			roofReferenceRoot = null; // never guess which building owns a satellite reference
...
//				globalUpdate.run();
		...
		modalPanel.add (modal = new MultiModalEditor( 
				...
				defaultReferenceFor( ns.klass ) ),
				BorderLayout.CENTER );
		
		modalPanel.revalidate();
	}

	private File defaultReferenceFor( Class<?> target ) {
		if ( target != RoofTexApp.class || roofReferenceRoot == null
				|| roofReferenceRoot.skelGen == null || roofReferenceRoot.skelGen.blockGen == null )
			return null;
		return ReferenceStyleSupport.readyRoofReference( roofReferenceRoot.skelGen.blockGen.getRoot() );
	}
