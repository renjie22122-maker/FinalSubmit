...
public class MultiModalEditor extends JPanel {
	...
	public MultiModalEditor( MultiModal mm, Class target, Runnable globalUpdate ) {
		this( mm, target, globalUpdate, null );
	}
...
//		out.add(gList, BorderLayout.CENTER);
	...
	public void addG(Runnable localUpdate, File meanImage ) {
		...
		if (meanImage == null) {
			...
			if ( SwingUtilities.isEventDispatchThread() )
				...
				SwingUtilities.invokeLater( addEmpty );
		}
		else {
			Runnable encode = () -> {
				ReferenceEncodeResult result = new Pix2Pix( NetInfo.get( target ) )
						.encodeStyleReference( meanImage, NetInfo.get( target ).sizeZ );
				if ( !result.succeeded() ) {
					showReferenceError( result.error );
					return;
				}
				SwingUtilities.invokeLater( () -> {
					Mode w = mm.newMode();
					w.ss.applyReferenceVector( result.latent, result.preview );
					GaussWrapper c = new GaussWrapper( w, localUpdate );
					gList.add( c );
					gList.revalidate();
					localUpdate.run();
			...
			if ( SwingUtilities.isEventDispatchThread() ) {
				Thread worker = new Thread( encode, "reference-style-mode-encoder" );
	...
	private void showReferenceError( String error ) {
		SwingUtilities.invokeLater( () -> JOptionPane.showMessageDialog( this, error,
				"Reference style was not changed", JOptionPane.ERROR_MESSAGE ) );
	}
...
//			setBorder( new LineBorder( Color.darkGray, 2, true ) );
			...
			add (w.ss.getUI( localUpdate, null, defaultReference ), BorderLayout.CENTER);
		}
