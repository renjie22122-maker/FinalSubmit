	...
	private static List<Line3d> slice(
		...
		for (int[] ind : mesh.faces) {
			...
			for (int t = 0; t < ind.length; t++) {
				...
				if ( !Double.isFinite( len ) || len <= GEOMETRY_EPSILON )
					continue;
				ab.normalize(); // needed?
...
//						add ( results, new Line(a[axisOrder[0]], a[axisOrder[1]], b[axisOrder[0]], b[axisOrder[1]])); // direction is randomly assigned
			...
			if ( !isFinite( normal ) || normal.lengthSquared() <= GEOMETRY_EPSILON * GEOMETRY_EPSILON )
				continue face;
			...
			if (pCount == 2) {
				...
				if ( !isFinite( lineDir ) || lineDir.lengthSquared() <= GEOMETRY_EPSILON * GEOMETRY_EPSILON )
					continue face;
